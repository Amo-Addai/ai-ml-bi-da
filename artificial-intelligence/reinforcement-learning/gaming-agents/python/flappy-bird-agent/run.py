...
class DQNAgent:
    ...
    def __init__(self):
        ...
        self.episodes = 1000
        self.state_space = self.env.state_observation_space.shape[0]
    ...
    def learn(self):
        ...
        for i in range(self.batch, number):
            state[i] = minibatch[0]
        ...
        for i in range(self.batch_number):
            if done[i]:
                ...
                target[i][action[i]] = rewared[i] + self.gamma * (np.argmax(target_next[i]))

            self.model.fit(state, target, batch_size=slef.batch_number, verbose=0)
